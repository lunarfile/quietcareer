...
const db = new Dexie('QuietCareerDB') as Dexie & {
  workLogs: EntityTable<WorkLog, 'id'>;
  energyCheckins: EntityTable<EnergyCheckin, 'id'>;
  financialData: EntityTable<FinancialData, 'id'>;
  goals: EntityTable<Goal, 'id'>;
  settings: EntityTable<Setting, 'key'>;
  bragDocuments: EntityTable<BragDocument, 'id'>;
  meetings: EntityTable<Meeting, 'id'>;
  weeklySnapshots: EntityTable<WeeklySnapshot, 'id'>;
};

db.version(1).stores({
  workLogs: 'id, date, impactType, project, *tags, createdAt',
  energyCheckins: 'id, date, level, createdAt',
  financialData: 'id, type, category, createdAt',
  goals: 'id, category, status, targetDate, createdAt',
  settings: 'key',
  bragDocuments: 'id, dateRangeStart, dateRangeEnd, createdAt',
});

db.version(2).stores({
  meetings: 'id, type, nextMeetingDate, createdAt',
  weeklySnapshots: 'id, weekStartDate, createdAt',
});

db.version(3).stores({
  workLogs: 'id, date, impactType, project, *tags, isPinned, createdAt',
}).upgrade((tx) => {
  return tx.table('workLogs').toCollection().modify((log) => {
    if (log.isPinned === undefined) log.isPinned = false;
  });
});
